...
export function Header() {
  ...
  return (
    <header className="w-full sticky top-0 left-0 right-0 bg-background z-[1] border-b h-[7vh] flex items-center justify-between px-32 md:px-10">
              ...
              {links.map((link) => (
                <li className="text-base font-medium" key={link.id}>
                  <Link
                    to={link.href}
                    className={clsx(
                      "hover:border-primary border-b-2 hover:text-primary py-2 px-2 transition-all",
                      {
                        "border-primary text-primary": link.href === pathname,
                        "border-transparent": link.href !== pathname,
                      },
                    )}
                  >
                    {link.title}
